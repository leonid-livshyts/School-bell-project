...
/* Portable C fallback.
 * Used on platforms without a hand-written asm path - notably the RP2040 /
 * RP2350 (Cortex-M0+ is Thumb-only ARMv6-M and has no smull, so the ARM asm
 * path above cannot be used). These are plain C and rely on the compiler /
 * libgcc for 64-bit math. */

typedef long long Word64;

static __inline int MULSHIFT32(int x, int y)
{
	/* signed multiply, return top 32 bits of the 64-bit result */
