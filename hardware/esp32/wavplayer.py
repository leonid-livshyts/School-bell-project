...
class WavPlayer:
    ...
    def __init__(self, id, sck_pin, ws_pin, sd_pin, ibuf, root="/sd", on_finish=None):
        self.id = id
        ...
        # allocate audio sample array buffer; keep this reasonably sized so socket reads
        # in the I2S callback are efficient without consuming too much memory
        self.wav_samples_mv = memoryview(bytearray(2048))
    ...
    def i2s_callback(self, arg):
        if self.state == WavPlayer.PLAY:
            try:
                ...
                print(f"ERROR in readinto: {e}")
                self.state = WavPlayer.STOP
                return
#             with open("file.txt", "a+") as f: 
#                 f.write(str(self.num_read) + "\n")
    
            # end of WAV file?
            if self.num_read == 0:
                # end-of-file
                print(self.nflush, "sss")
                if self.loop == False:
                ...
                try:
                    ...
                    print(f"ERROR in audio_out.write: {e}")
                    self.state = WavPlayer.STOP
    ...
    def parse(self, wav_file):
        ...
        offset = 0
        while len(buff) < 200:
            buff += wav_file.read(4)
            index = buff.find(b"data")
            if index >= 0:
                self.first_sample_offset = index
                break
            else:
                offset += 4
                
        
        if self.first_sample_offset is None:
            raise ValueError("WAV sub chunk 2 ID not found")
        
        self.first_sample_offset += 44
...
#         self.first_sample_offset = 44 + offset
    ...
    def play(self, wav_file=None, loop=False, wav_opened_file=None):
        if wav_opened_file is None and os.listdir(self.root).count(wav_file) == 0:
